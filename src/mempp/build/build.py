...
class TrajectoryBuilder(MemoryBuilder):
    ...
    def _extract_critical_actions(self, trajectory: Trajectory) -> List[Action]:
        ...
        critical = [
            ...
            if a.type in [ActionType.TOOL_USE, ActionType.PLANNING]
        ]
...
class ProceduralizationBuilder(MemoryBuilder):
    ...
    def _extract_key_patterns(self, traj_mem: TrajectoryMemory, 
                              script_mem: ScriptMemory) -> List[Dict[str, Any]]:
...
class PineconeMemoryStorage:
    ...
    async def store(self, memory: ProceduralMemory, namespace: Optional[str] = None) -> str:
        ...
        logger.info(f"Stored {memory.metadata.get('memory_type', 'unknown')} memory: {memory_id} in namespace: {namespace}")
        return memory_id
    ...
    def query(self, 
              ...
              sparse_vector: Optional[Dict[str, float]] = None) -> List[Dict]:
...
class MempBuildPipeline:
    ...
    async def build_from_trajectory(self, 
                                   trajectory: Trajectory,
                                   strategy: str = "proceduralization") -> ProceduralMemory:
    ...
    async def batch_build(self,
                         trajectories: List[Trajectory],
                         strategy: str = "proceduralization",
                         filter_successful: bool = True) -> List[ProceduralMemory]:
    ...
    def get_build_statistics(self) -> Dict[str, Any]:
        ...
        avg_build_time = (
            ...
            if self.build_stats['build_times'] else 0
        )
        ...
        return {
            **self.build_stats,
            ...
            'success_rate': (
                ...
                if self.build_stats['total_processed'] > 0 else 0
            ),
